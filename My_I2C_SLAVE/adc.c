...
//     unsigned char xdata k ;
   	 unsigned char idata counter=0;

	if(!adc_channels[mean_counter].new_measuring)
	{
		while(!adc_channels[mean_counter].new_measuring && counter<8)
		{
			mean_counter= (mean_counter+1)&0x7;
			counter++;
		}
		if(counter==8)
		{
	...
	channels[mean_counter].channel_data=Meaning((unsigned long)&adc_channels[mean_counter].ADC_BUF_UN,ADC_BUF_SIZE);  //????

   	adc_channels[mean_counter].new_measuring=0;
