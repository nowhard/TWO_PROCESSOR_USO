...
void I2C_Write_Buf(unsigned char addr,unsigned char *buf,unsigned char buf_len) //запись буфера в шину
{
	...
	byte_num=buf_len;
	i2c_buf=buf;
	transfer_byte=addr;//первым байтом передаем адрес
...
void I2C_Read_Buf(unsigned char addr,unsigned char *buf,unsigned char buf_len)  //чтение из шины в буфер
{
	...
	byte_num=buf_len;
	i2c_buf=buf;
	transfer_byte=addr|1;//0x80;//первым байтом передаем адрес
	i2c_st.GLOBAL_STATE=I2C_START;
	return;
}
//-------------------------------------------------------------
...
//--------------
				case 1:
				{
					...
					//{
					i2c_st.GLOBAL_STATE=I2C_RECIEVE_BYTE;//передаем следующий байт
					//}
