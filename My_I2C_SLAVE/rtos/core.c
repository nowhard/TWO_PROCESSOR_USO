...
//-----------------
 ...
 volatile iTimer  		idata  lTimer[maxTimers]; // список таймеров
// volatile iConstTimer   idata  lConstTimer[maxConstTimer];//список постоянных таймеров

 volatile iHandler  	idata lHandler[maxHandlers]; // список обработчиков

 volatile iMessage 		idata lMessage[maxMessages]; // буфер сообщений
...
void dispatchMessage(void) //using 2
 {	//возможно тормозит, оптимизировать
     char xdata  i=0;
  unsigned char   xdata res; //результат
  msg_num   xdata msg;
  msg_par   xdata par;
