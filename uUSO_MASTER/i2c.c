...
static PT_THREAD(I2C_Read_Complete(struct pt *pt))//постобработка пакета
{
   ...
   if(i2c_channels.I2C_CHNL.channels.CRC==CRC_Check(i2c_channels.I2C_CHNL.i2c_buf,10))
   {
	...
	switch(i2c_channels.I2C_CHNL.channels.frequency_modific)  //изменяем тип канала согласно ведомому
	{
		case  CHNL_FREQ_256:
		{
				channels[CHNL_FREQ_COUNT_T_0].channel_data=i2c_channels.I2C_CHNL.channels.frequency;
				channels[CHNL_FREQ_COUNT_T_0].settings.set.modific=CHNL_FREQ_256;
		...
		{
				channels[CHNL_FREQ_COUNT_T_0].channel_data=i2c_channels.I2C_CHNL.channels.frequency;
				channels[CHNL_FREQ_COUNT_T_1].channel_data=i2c_channels.I2C_CHNL.channels.mid_frequency;
